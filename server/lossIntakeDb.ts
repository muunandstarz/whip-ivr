...
export const DEFAULT_LOSS_INTAKE_SETTINGS = {
  ...
  intakeSlackDestinationChannelId: null,
  intakeSlackPublishingEnabled: false,
...
export async function upsertLossIntakeClaimBundle(input: {
  ...
  const values: InsertLossIntakeClaim = {
    ...
    vinLastSix: input.analysis.correctedVinLastSix ?? input.parent.vinLastSix,
    reportedVinLastSix: input.parent.vinLastSix,
    vinCorrectionEvidence: input.analysis.vinCorrectionEvidence,
    market: input.parent.market,
    vehicleType: input.parent.vehicleType,
    memberPhone: input.parent.memberPhone,
    preferredLanguage: input.parent.preferredLanguage,
    ...
    inspectionScheduledAt: input.analysis.inspectionScheduledAt,
    inspectionScheduleSource: input.analysis.inspectionScheduleSource,
  ...
  // Use VALUES() references in ON DUPLICATE KEY UPDATE to avoid doubling the packet size
  // (large text fields like factsOfLoss/rideshareStatus can push the payload over max_allowed_packet)
  await db.insert(lossIntakeClaims).values(values).onDuplicateKeyUpdate({
    set: {
      slackPermalink: sql`VALUES(\`slackPermalink\`)`,
      memberName: sql`VALUES(\`memberName\`)`,
      customerId: sql`VALUES(\`customerId\`)`,
      vinLastSix: sql`VALUES(\`vinLastSix\`)`,
      reportedVinLastSix: sql`VALUES(\`reported_vin_last_six\`)`,
      vinCorrectionEvidence: sql`VALUES(\`vin_correction_evidence\`)`,
      market: sql`VALUES(\`market\`)`,
      vehicleType: sql`VALUES(\`vehicleType\`)`,
      memberPhone: sql`VALUES(\`member_phone\`)`,
      preferredLanguage: sql`VALUES(\`preferred_language\`)`,
      assignedHandlerId: sql`VALUES(\`assignedHandlerId\`)`,
      assignedAgent: sql`VALUES(\`assignedAgent\`)`,
      stage: sql`VALUES(\`stage\`)`,
      hasPhotos: sql`VALUES(\`hasPhotos\`)`,
      attachmentCount: sql`VALUES(\`attachmentCount\`)`,
      firstContactAt: sql`VALUES(\`firstContactAt\`)`,
      firstContactMinutes: sql`VALUES(\`firstContactMinutes\`)`,
      slaState: sql`VALUES(\`slaState\`)`,
      slaType: sql`VALUES(\`slaType\`)`,
      slaDeadlineAt: sql`VALUES(\`slaDeadlineAt\`)`,
      completedAt: sql`VALUES(\`completedAt\`)`,
      intakeCycleMinutes: sql`VALUES(\`intakeCycleMinutes\`)`,
      factsOfLoss: sql`VALUES(\`factsOfLoss\`)`,
      preliminaryLiability: sql`VALUES(\`preliminaryLiability\`)`,
      rideshareStatus: sql`VALUES(\`rideshareStatus\`)`,
      noAnswerAttempts: sql`VALUES(\`noAnswerAttempts\`)`,
      contactAttempts: sql`VALUES(\`contactAttempts\`)`,
      dateOfLoss: sql`VALUES(\`dateOfLoss\`)`,
      templatePostedAt: sql`VALUES(\`templatePostedAt\`)`,
      templatePostMinutesFromContact: sql`VALUES(\`templatePostMinutesFromContact\`)`,
      templatePostMinutesFromReport: sql`VALUES(\`templatePostMinutesFromReport\`)`,
      storeTeamTagged: sql`VALUES(\`storeTeamTagged\`)`,
      folQualityScore: sql`VALUES(\`folQualityScore\`)`,
      ...
      inspectionScheduledAt: sql`VALUES(\`inspection_scheduled_at\`)`,
      inspectionScheduleSource: sql`VALUES(\`inspection_schedule_source\`)`,
