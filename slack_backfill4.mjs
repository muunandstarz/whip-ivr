...
const pool = mysql.createPool({ uri: DB_URL, connectionLimit: 5, connectTimeout: 30000, waitForConnections: true });
...
async function storagePut(relKey, buf, contentType) {
  ...
  const uploadResp = await fetch(s3Url, { method: 'PUT', headers: { 'Content-Type': contentType }, body: buf });
...
async function processFile(file) {
  ...
  try {
    ...
    try {
      await dbConn.execute('UPDATE mail_item_files SET storage_key = ?, slack_file_id = ? WHERE id = ?', [newKey, slackFileId, file.id]);
...
// Only process files NOT yet backfilled (storage_key doesn't start with mail/slack/backfill/)
const [files] = await pool.execute(`
  SELECT mf.id, mf.item_id, mf.storage_key, mf.filename, mf.content_type, mf.slack_file_id, mi.external_id
  FROM mail_item_files mf
  JOIN mail_items mi ON mi.id = mf.item_id
  WHERE mi.source = 'mail' AND (mf.storage_key NOT LIKE 'mail/slack/backfill/%' OR mf.storage_key IS NULL)
  ORDER BY mf.id
`);

console.log(`Found ${files.length} remaining Slack files to backfill. Processing in batches of ${BATCH}...`);
let ok = 0, failed = 0;
const failures = [];
