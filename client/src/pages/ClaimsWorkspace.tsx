...
type SceneData = { vehicles: SceneVehicle[]; marks: SceneMark[]; strokes: Stroke[] };
...
const DEFAULT_SCENE: SceneData = { vehicles: [], marks: [], strokes: [] };
...
function VehicleShape({ vehicle }: { vehicle: SceneVehicle }) {
  ...
  const width = vehicle.kind === "truck" || vehicle.kind === "van" ? 13 : vehicle.kind === "motorcycle" ? 5 : 10;
  const height = vehicle.kind === "truck" || vehicle.kind === "van" ? 20 : vehicle.kind === "motorcycle" ? 13 : 17;
  return <g transform={`translate(${x} ${y}) rotate(${vehicle.rotation})`}><rect x={-width / 2} y={-height / 2} width={width} height={height} rx="2" fill={vehicle.color} stroke="white" strokeWidth=".8" /><path d={`M${-width / 2 + 1.3} ${-height / 2 + 4} H${width / 2 - 1.3}`} stroke="white" strokeWidth="1.2" opacity=".8" /><circle cx={-width / 2 - .5} cy={-height / 3} r="1" fill="#25303e" /><circle cx={width / 2 + .5} cy={-height / 3} r="1" fill="#25303e" /><circle cx={-width / 2 - .5} cy={height / 3} r="1" fill="#25303e" /><circle cx={width / 2 + .5} cy={height / 3} r="1" fill="#25303e" /></g>;
}

function SceneSvg({ layout, scene, showRoad = true, onPointer, onVehiclePointerDown }: { layout: RoadLayout; scene: SceneData; showRoad?: boolean; onPointer: (event: React.PointerEvent<SVGSVGElement>) => void; onVehiclePointerDown: (event: React.PointerEvent<SVGGElement>, id: string) => void }) {
  return <svg id="claims-scene-svg" viewBox="0 0 100 100" preserveAspectRatio="none" className="w-full h-full touch-none select-none" onPointerDown={onPointer} onPointerMove={onPointer} onPointerUp={onPointer}>
    ...
    {scene.vehicles.map(vehicle => <g key={vehicle.id} onPointerDown={event => onVehiclePointerDown(event, vehicle.id)} className="cursor-grab active:cursor-grabbing"><VehicleShape vehicle={vehicle} /><text x={vehicle.x} y={vehicle.y + 13} textAnchor="middle" fontSize="2.6" fill="#172033" fontWeight="600" stroke="white" strokeWidth=".7" paintOrder="stroke">{vehicle.label}</text></g>)}
  </svg>;
}
...
function AccidentWorkspace({ scenes }: { scenes: any[] }) {
  ...
  useEffect(() => {
    ...
    setLayout(selectedScene.roadLayout as RoadLayout); setScene(getSceneData(selectedScene.sceneData));
  }, [selectedSceneId]);

  const addVehicle = (kind: VehicleKind) => {
    const entry = VEHICLE_KINDS.find(vehicle => vehicle.value === kind)!;
    setScene(current => ({ ...current, vehicles: [...current.vehicles, { id: crypto.randomUUID(), kind, label: kind === "sedan" ? "IV" : "CV", x: 50, y: 50, rotation: 0, color: entry.color }] }));
  };
  ...
  const handlePointer = (event: React.PointerEvent<SVGSVGElement>) => {
    ...
    if (event.type === "pointerup") { setDraggedVehicle(null); setCurrentStroke(null); }
  };
  ...
  const geocode = () => { if (!mapRef.current || !location.trim() || !window.google) return; new google.maps.Geocoder().geocode({ address: location }, (results, status) => { if (status === "OK" && results?.[0]) { mapRef.current!.setCenter(results[0].geometry.location); mapRef.current!.setZoom(18); } else toast.error("Location not found. Refine the loss location and try again."); }); };
  ...
  return <div className="space-y-5">
    ...
    <div className="grid xl:grid-cols-[220px_minmax(0,1fr)_330px] gap-5">
      <Card className="border-slate-200 shadow-sm h-fit"><CardHeader className="pb-3"><CardTitle className="text-sm">Scene tools</CardTitle></CardHeader><CardContent className="space-y-4">
        ...
        <div><p className="mb-2 text-xs font-medium">Add road controls</p><div className="grid grid-cols-2 gap-2">{[["stop", "Stop sign"], ["light", "Traffic light"], ["crosswalk", "Crosswalk"], ["impact", "Impact"], ["arrow", "Arrow"], ["note", "Label"]].map(([kind, label]) => <Button key={kind} variant="outline" size="sm" className="text-xs justify-start" onClick={() => addMark(kind as SceneMark["kind"])}>{label}</Button>)}</div></div>
      ...
      <Card className="border-slate-200 shadow-sm overflow-hidden"><CardHeader className="pb-3 flex-row items-center justify-between space-y-0"><div><CardTitle className="text-sm">Accident Workspace</CardTitle><CardDescription>Drag vehicles, rotate them in the scene list, and sketch paths or evidence.</CardDescription></div><Tabs value={mode} onValueChange={value => setMode(value as "diagram" | "map")}><TabsList className="h-8"><TabsTrigger value="diagram" className="text-xs h-7"><Pencil className="mr-1 h-3 w-3" />Diagram</TabsTrigger><TabsTrigger value="map" className="text-xs h-7"><MapIcon className="mr-1 h-3 w-3" />Map</TabsTrigger></TabsList></Tabs></CardHeader>
        <CardContent className="p-0"><div className="relative aspect-[4/3] overflow-hidden bg-slate-100">{mode === "diagram" ? <SceneSvg layout={layout} scene={scene} onPointer={handlePointer} onVehiclePointerDown={(event, id) => { event.stopPropagation(); setDraggedVehicle(id); (event.currentTarget.ownerSVGElement as SVGSVGElement)?.setPointerCapture(event.pointerId); }} /> : <div className="relative h-full"><MapView className="h-full" initialCenter={{ lat: 39.084, lng: -77.152 }} initialZoom={12} onMapReady={map => { mapRef.current = map; }} /><div className="absolute inset-0 pointer-events-none bg-black/5" /><div className="absolute top-3 left-3 right-3 flex gap-2"><Input className="bg-white shadow-sm" value={location} onChange={event => setLocation(event.target.value)} placeholder="Paste loss location or intersection" /><Button onClick={geocode} className="shrink-0"><Search className="h-4 w-4" /></Button></div><div className="absolute inset-0 top-14"><SceneSvg layout={layout} showRoad={false} scene={scene} onPointer={handlePointer} onVehiclePointerDown={(event, id) => { event.stopPropagation(); setDraggedVehicle(id); }} /></div></div>}</div>
          {scene.vehicles.length > 0 && <div className="border-t p-3 flex flex-wrap gap-2">{scene.vehicles.map(vehicle => <div key={vehicle.id} className="flex items-center gap-1 rounded-lg border bg-white px-2 py-1 text-xs"><span className="h-2 w-2 rounded-full" style={{ backgroundColor: vehicle.color }} /><Input aria-label={`${vehicle.label} label`} className="h-6 w-14 px-1 text-xs border-0" value={vehicle.label} onChange={event => setScene(current => ({ ...current, vehicles: current.vehicles.map(item => item.id === vehicle.id ? { ...item, label: event.target.value } : item) }))} /><input aria-label={`${vehicle.label} rotation`} className="w-16 accent-violet-600" type="range" min="0" max="359" value={vehicle.rotation} onChange={event => setScene(current => ({ ...current, vehicles: current.vehicles.map(item => item.id === vehicle.id ? { ...item, rotation: Number(event.target.value) } : item) }))} /><button onClick={() => setScene(current => ({ ...current, vehicles: current.vehicles.filter(item => item.id !== vehicle.id) }))}><X className="h-3.5 w-3.5 text-muted-foreground" /></button></div>)}</div>}
        </CardContent></Card>
