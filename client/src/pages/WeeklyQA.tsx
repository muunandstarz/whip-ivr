...
import { AlertTriangle, ArrowRight, BookOpenCheck, Check, ChevronRight, CircleAlert, ClipboardCheck, ExternalLink, FileText, Gavel, Info, MessageSquare, Plus, Scale, Send, ShieldCheck, Sparkles, TrendingUp, UserCheck, Users } from 'lucide-react';
...
function EvaluationDetail({ evaluationId, open, onOpenChange, leadership }: { evaluationId: number | null; open: boolean; onOpenChange: (value: boolean) => void; leadership: boolean }) {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="w-[calc(100vw-1rem)] !max-w-none h-[calc(100dvh-1rem)] overflow-hidden p-0 sm:w-[calc(100vw-2rem)] sm:!max-w-6xl"><DialogHeader className="border-b px-4 py-4 pr-12 text-left sm:px-6"><div className="flex flex-wrap items-center justify-between gap-3"><div><DialogTitle className="text-xl">{evaluation?.handler_name ?? 'Claims QA evaluation'}</DialogTitle><DialogDescription>{auditDisplayKey(evaluation)} · {evaluation?.role} · Claim {evaluation?.claim_number || 'not supplied'} · Audited {displayDate(evaluation?.audit_date)}</DialogDescription></div>{evaluation && <div className="flex items-center gap-2"><RatingBadge value={evaluation.original_rating} /><Badge variant="outline">{statusLabel(evaluation.status)}</Badge></div>}</div></DialogHeader><div className="min-h-0 overflow-y-auto p-4 sm:p-6">{detailQuery.isLoading ? <p className="p-8 text-center text-muted-foreground">Loading full scorecard…</p> : !evaluation ? <div className="mx-auto max-w-lg py-20 text-center"><p className="text-lg font-semibold">Evaluation unavailable in Claims QA</p><p className="mt-2 text-sm text-muted-foreground">Preserved AI call-quality records are intentionally available only from Call Tracking.</p><Button className="mt-5" variant="outline" onClick={() => window.location.assign('/call-tracking')}>Open Call Tracking</Button></div> : <div className="space-y-5 pb-4"><div className="grid gap-3 grid-cols-[repeat(auto-fit,minmax(min(100%,210px),1fr))]"><Metric label="Rating" value={ratingLabel(evaluation.original_rating)} caption={source?.scoreInWords ?? 'Critical-first headline'} tone={evaluation.original_rating === 'critical_miss' ? 'red' : 'navy'} /><Metric label="Score" value={evaluation.original_items_scored ? `${evaluation.original_items_met} / ${evaluation.original_items_scored}` : '—'} caption={evaluation.original_pass_rate == null ? 'Percentage suppressed for small sample' : `${evaluation.original_pass_rate}% pass rate`} tone="green" /><Metric label="Critical failures" value={evaluation.original_critical_failures} caption="Never averaged away" tone={evaluation.original_critical_failures ? 'red' : 'green'} /><Metric label="N/A lines" value={`${evaluation.original_not_applicable} + ${evaluation.original_not_determinable}`} caption="Not applicable + not determinable" tone="orange" /></div>
      {(evaluation.auditor_summary || evaluation.areas_for_improvement || source?.strengths) && <div className="grid gap-3 grid-cols-[repeat(auto-fit,minmax(min(100%,250px),1fr))]"><Card><CardHeader className="pb-2"><CardTitle className="text-sm">Auditor summary</CardTitle></CardHeader><CardContent className="text-sm whitespace-pre-wrap leading-relaxed">{evaluation.auditor_summary || 'No summary supplied.'}</CardContent></Card><Card className="border-emerald-200"><CardHeader className="pb-2"><CardTitle className="text-sm text-emerald-900">Strengths</CardTitle></CardHeader><CardContent className="text-sm whitespace-pre-wrap leading-relaxed">{source?.strengths || 'No strengths summary supplied.'}</CardContent></Card><Card className="border-amber-200"><CardHeader className="pb-2"><CardTitle className="text-sm text-amber-900">Areas for improvement</CardTitle></CardHeader><CardContent className="text-sm whitespace-pre-wrap leading-relaxed">{evaluation.areas_for_improvement || 'No scored misses in this file.'}</CardContent></Card></div>}
...
function EvaluationTable({ leadership, onOpen, onOpenPage }: { leadership: boolean; onOpen: (id: number) => void; onOpenPage: (id: number) => void }) {
  const [role, setRole] = useState('all');
  const [status, setStatus] = useState('all');
  const [handlerId, setHandlerId] = useState('all');
  const [search, setSearch] = useState('');
  const { data: handlerOptions } = trpc.handlers.list.useQuery(undefined, { enabled: leadership });
  const { data: evaluations, isLoading } = trpc.claimsQa.list.useQuery({
    role: role === 'all' ? undefined : role,
    status: status === 'all' ? undefined : status,
    handlerId: leadership && handlerId !== 'all' ? Number(handlerId) : undefined,
    limit: 300,
  });
...
export default function WeeklyQA() {
  ...
  const [overviewHandlerId, setOverviewHandlerId] = useState('all');
  const routeEvaluation = useMemo(() => {
    const match = location.match(/^\/(?:claims-qa|qa)\/audit\/(\d+)$/);
    return match ? Number(match[1]) : null;
  }, [location]);
  const overview = trpc.claimsQa.overview.useQuery({ handlerId: leadership && overviewHandlerId !== 'all' ? Number(overviewHandlerId) : undefined });
  const data = overview.data as any;
  const open = (id: number) => setSelectedEvaluation(id);
  const openInPage = (id: number) => window.open(`/claims-qa/audit/${id}`, '_blank', 'noopener,noreferrer');
  ...
  return <WhipLayout><div className="p-4 sm:p-6 space-y-5"><header className="flex flex-col gap-3 border-b pb-5 lg:flex-row lg:items-end lg:justify-between"><div><div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-[#d94d13]"><ShieldCheck className="h-4 w-4" /> Claims quality programme</div><h1 className="mt-2 text-2xl font-bold tracking-tight sm:text-3xl">Claims QA</h1><p className="mt-1 max-w-3xl text-sm text-muted-foreground">Evidence-led scorecards, human-confirmed critical findings, handler response, adjudication, coaching, and calibration. Quality is kept separate from call productivity.</p></div>{leadership && <Button className="bg-[#ff6221] hover:bg-[#e5541a] text-white gap-2" onClick={() => setCreateOpen(true)}><Plus className="h-4 w-4" /> New evaluation</Button>}</header>
    <Tabs value={tab} onValueChange={setTab} className="gap-4"><TabsList className="h-auto w-full justify-start overflow-x-auto bg-muted p-1 sm:w-fit"><TabsTrigger value="overview" className="px-3"><ClipboardCheck /> Overview</TabsTrigger><TabsTrigger value="evaluations" className="px-3"><FileText /> Evaluations</TabsTrigger><TabsTrigger value="scorecard" className="px-3"><BookOpenCheck /> Scorecard</TabsTrigger>{leadership && <TabsTrigger value="disputes" className="px-3"><Gavel /> Disputes</TabsTrigger>}{leadership && <TabsTrigger value="calibration" className="px-3"><Scale /> Calibration</TabsTrigger>}</TabsList>
      <TabsContent value="overview" className="space-y-5">{overview.isLoading ? <p className="py-10 text-center text-sm text-muted-foreground">Loading Claims QA…</p> : <><div className="flex flex-col gap-3 rounded-xl border border-primary/15 bg-primary/[0.03] p-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="font-medium">Handler progress and recurring trends</p><p className="mt-1 text-sm text-muted-foreground">Review one handler at a time or the complete quality programme. Trend percentages follow the same small-sample rule.</p></div>{leadership && <div className="w-full sm:w-64"><Label className="mb-1 block text-xs">Handler</Label><Select value={overviewHandlerId} onValueChange={setOverviewHandlerId}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">All handlers</SelectItem>{(data?.availableHandlers ?? []).map((handler: any) => <SelectItem key={handler.handlerId} value={String(handler.handlerId)}>{handler.handlerName}</SelectItem>)}</SelectContent></Select></div>}</div><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><Metric label="Completed audits" value={data?.counts?.evaluations ?? 0} caption="Claim-file evaluations in Claims QA" /><Metric label="Pass rate" value={data?.quality?.passRate == null ? '—' : `${data.quality.passRate}%`} caption={data?.quality?.scored ? `${data.quality.met} of ${data.quality.scored} scored items met` : 'No scored claim lines yet'} tone="green" /><Metric label="Critical misses" value={data?.counts?.criticalFailures ?? 0} caption="Never averaged away" tone={data?.counts?.criticalFailures ? 'red' : 'green'} /><Metric label="Files clean" value={data?.counts?.filesClean ?? 0} caption="Strong with no critical miss" tone="orange" /></div><div className="grid gap-5 xl:grid-cols-[1.2fr_1fr]"><Card><CardHeader><CardTitle className="text-base">Quality by scorecard section</CardTitle><CardDescription>Worst actionable category first. <Definition label="Pass rate" body={data?.definitions?.passRate ?? ''} /></CardDescription></CardHeader><CardContent>{(data?.categories ?? []).length ? <div className="space-y-3">{data.categories.map((row: any) => <div key={row.category} className="rounded-lg border p-3"><div className="flex items-center justify-between gap-3"><p className="text-sm font-medium">{row.category}</p><span className="text-sm font-semibold">{row.passRate == null ? 'Small sample' : `${row.passRate}%`}</span></div><p className="mt-1 text-xs text-muted-foreground">{row.met} of {row.scored} met · {row.criticalFailures} critical</p></div>)}</div> : <p className="rounded-lg bg-muted/40 p-5 text-sm text-muted-foreground">Category results will appear after claim evaluations are scored.</p>}</CardContent></Card><Card><CardHeader><CardTitle className="text-base">Most-missed items</CardTitle><CardDescription>Training and rubric signals, not a handler leaderboard.</CardDescription></CardHeader><CardContent>{(data?.mostMissed ?? []).length ? <div className="space-y-3">{data.mostMissed.map((row: any) => <div key={row.itemKey} className="rounded-lg bg-muted/40 p-3"><div className="flex items-start justify-between gap-3"><p className="font-mono text-xs text-primary">{row.itemKey}</p><span className="text-xs font-semibold text-red-700">{row.missRate}% miss rate</span></div><p className="mt-1 text-sm">{row.checkText}</p><p className="mt-1 text-xs text-muted-foreground">{row.misses} misses across {row.scored} scored audits</p></div>)}</div> : <p className="rounded-lg bg-muted/40 p-5 text-sm text-muted-foreground">No claim-level misses have been scored.</p>}</CardContent></Card></div><div className="grid gap-5 xl:grid-cols-[1.2fr_1fr]"><Card><CardHeader><CardTitle className="flex items-center gap-2 text-base"><TrendingUp className="h-4 w-4 text-primary" /> Month-over-month progress</CardTitle><CardDescription>Monthly scored-item performance; a percentage is intentionally hidden below 15 scored lines.</CardDescription></CardHeader><CardContent>{(data?.progress ?? []).length ? <div className="space-y-2">{data.progress.map((row: any) => <div key={`${row.handlerId}-${row.month}`} className="flex flex-wrap items-center justify-between gap-3 rounded-lg border p-3"><div><p className="font-medium">{row.handlerName}</p><p className="text-xs text-muted-foreground">{row.month} · {row.evaluations} audit{row.evaluations === 1 ? '' : 's'} · {row.met}/{row.scored} met</p></div><div className="text-right"><p className="font-semibold">{row.passRate == null ? 'Small sample' : `${row.passRate}%`}</p><p className="text-xs text-muted-foreground">{row.criticalFailures} critical</p></div></div>)}</div> : <p className="rounded-lg bg-muted/40 p-5 text-sm text-muted-foreground">Month-over-month progress will populate as completed audits span more than one period.</p>}</CardContent></Card><Card><CardHeader><CardTitle className="text-base">Repeated misses</CardTitle><CardDescription>Items missed at least twice for the selected view, so coaching can focus on persistent gaps.</CardDescription></CardHeader><CardContent>{(data?.repeatedMisses ?? []).length ? <div className="space-y-2">{data.repeatedMisses.map((row: any) => <div key={`${row.handlerId}-${row.itemKey}`} className="rounded-lg border border-red-100 bg-red-50/40 p-3"><div className="flex flex-wrap items-start justify-between gap-2"><div><p className="font-mono text-xs text-primary">{row.itemKey}</p><p className="mt-1 text-sm font-medium">{row.checkText}</p></div><Badge variant="outline" className="border-red-200 text-red-700">{row.misses} misses</Badge></div><p className="mt-2 text-xs text-muted-foreground">{row.handlerName} · {row.missRate}% miss rate across {row.scored} scored instances · last observed {displayDate(row.lastObserved)}</p></div>)}</div> : <p className="rounded-lg bg-muted/40 p-5 text-sm text-muted-foreground">No repeated claim-QA misses are present for this view.</p>}</CardContent></Card></div><Card><CardHeader><CardTitle className="text-base">{leadership ? 'Handler quality view' : 'Your quality view'}</CardTitle><CardDescription>Quality only. Call performance and productivity remain in Call Tracking. <Definition label="Critical failures" body={data?.definitions?.criticalFailures ?? ''} /></CardDescription></CardHeader><CardContent className="p-0">{(data?.handlers ?? []).length ? <div className="overflow-x-auto"><table className="w-full min-w-[700px] text-sm"><thead className="border-y bg-muted/30 text-left text-xs text-muted-foreground"><tr><th className="px-4 py-3">Handler</th><th className="px-4 py-3">Evaluations</th><th className="px-4 py-3">Pass rate</th><th className="px-4 py-3">Critical</th><th className="px-4 py-3">Open actions</th></tr></thead><tbody className="divide-y">{data.handlers.map((row: any) => <tr key={row.handlerId}><td className="px-4 py-3 font-medium">{row.handlerName}</td><td className="px-4 py-3">{row.evaluations}</td><td className="px-4 py-3">{row.passRate == null ? 'Small sample' : `${row.passRate}%`} <span className="text-xs text-muted-foreground">({row.met}/{row.scored})</span></td><td className="px-4 py-3">{row.criticalFailures}</td><td className="px-4 py-3">{row.openActions}</td></tr>)}</tbody></table></div> : <p className="p-5 text-sm text-muted-foreground">No visible evaluation records.</p>}</CardContent></Card></>}</TabsContent>
      <TabsContent value="evaluations"><EvaluationTable leadership={leadership} onOpen={open} onOpenPage={openInPage} /></TabsContent>
    ...
    <EvaluationDetail evaluationId={routeEvaluation ?? selectedEvaluation} open={routeEvaluation !== null || selectedEvaluation !== null} onOpenChange={(next) => { if (!next) { if (routeEvaluation !== null) navigate('/claims-qa'); else setSelectedEvaluation(null); } }} leadership={leadership} />
  </div></WhipLayout>;
}
