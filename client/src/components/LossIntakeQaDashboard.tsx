    ...
    <div className="flex flex-col gap-5 xl:flex-row">
      ...
      <div className="min-w-0 flex-1 space-y-5">
        {page === "brief" && <>
          ...
          <div className="grid gap-4 xl:grid-cols-2"><Card><CardHeader><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-[0.12em] text-muted-foreground">Response by work type</p><CardTitle className="mt-1 text-lg">Two queues, two standards</CardTitle></div><Status tone="neutral">Business hours</Status></div></CardHeader><CardContent className="space-y-5"><div><div className="flex items-center justify-between gap-4"><div><p className="text-xs font-semibold text-[#527fdb]">IN-OFFICE / #CLAIMS</p><p className="mt-1 text-sm text-muted-foreground">Vehicle photos trigger the 10-minute statement-attempt standard.</p></div><strong className="text-xl text-foreground">{officeOnTime == null ? "—" : `${officeOnTime}%`}</strong></div><Progress value={officeOnTime} tone="bg-[#527fdb]" /></div><div><div className="flex items-center justify-between gap-4"><div><p className="text-xs font-semibold text-emerald-700">REMOTE / #CLAIMS-REMOTEMARKETS</p><p className="mt-1 text-sm text-muted-foreground">Weekday 9 AM–6 PM ET clock with a 4-business-hour completion standard.</p></div><strong className="text-xl text-foreground">{remoteOnTime == null ? "—" : `${remoteOnTime}%`}</strong></div><Progress value={remoteOnTime} tone="bg-emerald-500" /></div></CardContent></Card>
